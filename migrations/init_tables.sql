...
-- 飞书 / 钉钉 / 企业微信 / Slack / 通用 Webhook 相关表全部移除（新装以 Telegram 为唯一通知渠道；
-- 存量库依次执行 patch_203_drop_feishu_legacy.sql、patch_205_drop_dingtalk_legacy.sql、
-- patch_206_drop_wechatwork_and_unified_channels.sql）。
...
CREATE TABLE IF NOT EXISTS `alert_configs` (
  ...
  `type` varchar(50) NOT NULL COMMENT '告警类型: k8s/healthcheck/custom',
  ...
  `notify_channels` varchar(500) DEFAULT '' COMMENT '通知渠道: telegram,webhook,email 等',
...
CREATE TABLE IF NOT EXISTS `applications` (
  ...
  `description` varchar(500) DEFAULT '' COMMENT '描述',
  `organization_id` bigint unsigned DEFAULT NULL COMMENT '所属组织',
  `project_id` bigint unsigned DEFAULT NULL COMMENT '所属项目',
  ...
  KEY `idx_app_org` (`organization_id`),
  KEY `idx_app_proj` (`project_id`)
...
CREATE TABLE IF NOT EXISTS `application_envs` (
  ...
  `gitops_repo_id` bigint unsigned DEFAULT NULL COMMENT 'GitOps部署仓库ID',
  `argocd_application_id` bigint unsigned DEFAULT NULL COMMENT 'ArgoCD应用ID',
  `gitops_branch` varchar(200) DEFAULT '' COMMENT 'GitOps目标分支',
  `gitops_path` varchar(500) DEFAULT '' COMMENT 'GitOps部署目录',
  ...
  KEY `idx_app_env_k8s_cluster` (`k8s_cluster_id`),
  KEY `idx_app_env_gitops_repo` (`gitops_repo_id`),
  KEY `idx_app_env_argocd_app` (`argocd_application_id`)
...
CREATE TABLE IF NOT EXISTS `deploy_records` (
  ...
  `deploy_method` varchar(50) DEFAULT 'k8s' COMMENT '部署方式: k8s/gitops',
...
CREATE TABLE IF NOT EXISTS `approval_instances` (
  ...
  `target_type` varchar(20) NOT NULL DEFAULT 'deploy' COMMENT '业务对象: deploy/sql_change',
  `target_id` bigint unsigned DEFAULT NULL COMMENT '业务对象ID',
  `chain_id` bigint unsigned NOT NULL COMMENT '审批链ID',
...
('operator', '运维人员', '可以操作 K8s 等资源', 1, 'active'),
...
CREATE TABLE IF NOT EXISTS `oa_notify_configs` (
  ...
  `app_id` bigint unsigned DEFAULT NULL COMMENT '关联应用ID（历史遗留字段）',
...
CREATE TABLE IF NOT EXISTS `pipelines` (
  ...
  `env` varchar(50) DEFAULT '' COMMENT '交付环境',
  `source_template_id` bigint unsigned DEFAULT NULL COMMENT '来源模板ID',
  `git_repo_id` bigint unsigned DEFAULT NULL COMMENT 'Git 仓库ID',
  `git_branch` varchar(100) DEFAULT 'main' COMMENT 'Git 分支',
  `config_json` longtext NOT NULL COMMENT '流水线配置 JSON',
  `trigger_config_json` text COMMENT '触发配置 JSON',
  `trigger_config` text COMMENT 'Webhook 触发配置',
  `status` varchar(20) DEFAULT 'active' COMMENT '状态: active/inactive',
  `last_run_at` datetime(3) DEFAULT NULL COMMENT '最近运行时间',
  `last_run_status` varchar(20) DEFAULT '' COMMENT '最近运行状态',
  `created_by` bigint unsigned DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_p_deleted_at` (`deleted_at`),
  KEY `idx_p_project_id` (`project_id`),
  KEY `idx_p_application_id` (`application_id`),
  KEY `idx_p_application_name` (`application_name`),
  KEY `idx_p_env` (`env`),
  KEY `idx_p_source_template_id` (`source_template_id`),
  KEY `idx_p_git_repo_id` (`git_repo_id`),
...
CREATE TABLE IF NOT EXISTS `pipeline_runs` (
  ...
  KEY `idx_pr_application_name` (`application_name`),
  KEY `idx_pr_env` (`env`),
...
-- 125. LDAP 组→角色映射（与 upgrades.sql 一致）
...
-- 126. Telegram（与 upgrades.sql 一致）
