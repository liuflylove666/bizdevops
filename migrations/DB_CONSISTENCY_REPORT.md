...
**生成时间**: 2026-04-14  
**检查范围**: 全部数据库表与对应的 Go Model  
**问题总数**: 23 个（严重 6 个，中等 6 个，缺失表 11 个）

> ⚠️ 历史报告：文中涉及的**飞书（Feishu）**、**钉钉（DingTalk）**、**Jenkins** 相关检查项
> 已随这三项集成被整体下线而自然失效，请勿据此修复。
> 现行结构请以 `init_tables.sql` 为准。
...
**问题描述**:
- SQL 字段名: `env`, `k8s_cluster_id`, `k8s_namespace`, `k8s_deployment`
- Model 字段名: `env_name`, `branch`，且缺少 `k8s_cluster_id`
