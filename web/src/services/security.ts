...
export interface ImageScanHistoryItem {
  id: number
  image: string
...
// 镜像扫描
export function scanImage(data: { image: string; registry_id?: number; application_id?: number; application_name?: string; pipeline_run_id?: number }) {
  return request.post('/security/scan', data)
}

export function getScanHistory(params?: { image?: string; status?: string; application_id?: number; application_name?: string; pipeline_run_id?: number; page?: number; page_size?: number }) {
