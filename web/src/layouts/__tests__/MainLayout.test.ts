...
describe('Menu State Management', () => {
  ...
  describe('getParentKey', () => {
    const getParentKey = (path: string): string => {
      if (path.startsWith('/applications') || path.startsWith('/deploy/check')) return 'app'
      ...
      if (path.startsWith('/telegram')) return 'message'
    ...
    it('should return correct parent key for pipeline routes', () => {
      ...
      expect(getParentKey('/pipeline/stats')).toBe('pipeline')
    })
