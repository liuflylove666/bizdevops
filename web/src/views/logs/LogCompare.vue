...
const syncGlobalFiltersToRoute = () => {
  ...
  if (compareForm.cluster_id) query.cluster_id = String(compareForm.cluster_id)
  else delete query.cluster_id
  if (compareForm.namespace) query.namespace = compareForm.namespace
...
onMounted(() => {
  loadClusters().then(async () => {
    ...
    if (cluster) {
      compareForm.cluster_id = cluster
      await onClusterChange()
      if (ns) {
        compareForm.namespace = ns
...
watch(
  ...
  async () => {
    const cluster = parseRouteCluster()
    const ns = typeof route.query.namespace === 'string' ? route.query.namespace : ''
    if (cluster !== compareForm.cluster_id) {
      compareForm.cluster_id = cluster
      if (cluster) await onClusterChange()
    }
    if (ns !== compareForm.namespace) {
      compareForm.namespace = ns
