...
const syncGlobalFiltersToRoute = () => {
  const query = { ...route.query } as Record<string, any>
  if (searchForm.value.cluster_id) query.cluster_id = String(searchForm.value.cluster_id)
  else delete query.cluster_id
  if (searchForm.value.namespace) query.namespace = searchForm.value.namespace
  else delete query.namespace
...
onMounted(() => {
  loadClusters().then(async () => {
    const cluster = parseRouteCluster()
    const ns = typeof route.query.namespace === 'string' ? route.query.namespace : ''
    if (cluster) {
      searchForm.value.cluster_id = cluster
      await onClusterChange()
      if (ns) {
        searchForm.value.namespace = ns
        await onNamespaceChange()
      }
    }
  })
...
watch(
  ...
  async () => {
    const cluster = parseRouteCluster()
    const ns = typeof route.query.namespace === 'string' ? route.query.namespace : ''
    if (cluster !== searchForm.value.cluster_id) {
      searchForm.value.cluster_id = cluster
      if (cluster) await onClusterChange()
    }
    if (ns !== searchForm.value.namespace) {
      searchForm.value.namespace = ns
      if (cluster && ns) await onNamespaceChange()
    }
  },
)
</script>
