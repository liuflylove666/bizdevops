...
const syncGlobalFiltersToRoute = () => {
  ...
  if (form.cluster_id) query.cluster_id = String(form.cluster_id)
  else delete query.cluster_id
  if (form.namespace) query.namespace = form.namespace
...
const startExport = async () => {
  ...
  try {
    ...
    exportTask.value = res.data || null
    if (exportTask.value) startPolling()
  } catch (error: any) {
...
const startPolling = () => {
  ...
  pollTimer = window.setInterval(async () => {
    ...
    try {
      ...
      exportTask.value = nextTask || null
...
watch(
  ...
  async () => {
    const cluster = parseRouteCluster()
    const ns = typeof route.query.namespace === 'string' ? route.query.namespace : ''
    if (cluster !== form.cluster_id) {
      form.cluster_id = cluster
      if (cluster) await onClusterChange()
    }
    if (ns !== form.namespace) {
      form.namespace = ns
