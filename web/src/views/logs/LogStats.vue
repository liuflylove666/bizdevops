...
const syncGlobalFiltersToRoute = () => {
  ...
  if (filterForm.cluster_id) query.cluster_id = String(filterForm.cluster_id)
  else delete query.cluster_id
  if (filterForm.namespace) query.namespace = filterForm.namespace
...
onMounted(() => {
  loadClusters().then(async () => {
    ...
    if (cluster) {
      filterForm.cluster_id = cluster
      await onClusterChange()
      if (ns) filterForm.namespace = ns
    }
  })
...
watch(
  ...
  async () => {
    const cluster = parseRouteCluster()
    const ns = typeof route.query.namespace === 'string' ? route.query.namespace : ''
    if (cluster !== filterForm.cluster_id) {
      filterForm.cluster_id = cluster
      if (cluster) await onClusterChange()
    }
    if (ns !== filterForm.namespace) {
      filterForm.namespace = ns
    }
  },
)
