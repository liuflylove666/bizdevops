    ...
    <a-modal v-model:open="showAddDialog" :title="editingRule ? '编辑规则' : '添加规则'" width="450px" @ok="saveRule">
      <a-form :model="ruleForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
        ...
        <a-form-item label="预览">
          <div class="rule-preview-large" :style="{ color: normalizeColor(ruleForm.fg_color), backgroundColor: normalizeColor(ruleForm.bg_color) }">
...
const ruleForm = ref({
  ...
  match_value: '',
  fg_color: '#FFFFFF' as ColorValue,
  bg_color: '#DC3545' as ColorValue,
  priority: 50,
  enabled: true
})

const normalizeColor = (value: ColorValue) => typeof value === 'string' ? value : value.toHexString()
...
const saveRule = async () => {
  ...
  const formToSave = {
    ...ruleForm.value,
    fg_color: normalizeColor(ruleForm.value.fg_color),
    bg_color: normalizeColor(ruleForm.value.bg_color)
  }
