...
const syncGlobalFiltersToRoute = () => {
  ...
  if (selectedCluster.value) query.cluster_id = String(selectedCluster.value)
...
const selectPod = (pod: Pod) => {
  ...
  selectedContainer.value = firstContainer || ''
}
...
watch(
  ...
  async () => {
    ...
    if (cluster !== selectedCluster.value) {
      selectedCluster.value = cluster
      if (cluster) await loadNamespaces()
    }
    if (ns !== selectedNamespace.value) {
      selectedNamespace.value = ns
      if (cluster && ns) await loadPods()
    }
  },
)
</script>
