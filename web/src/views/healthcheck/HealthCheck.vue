    ...
    <a-modal v-model:open="configModalVisible" :title="editingConfigId ? '编辑检查配置' : '添加检查配置'" @ok="saveConfig" :confirm-loading="savingConfig" width="600px">
      <a-form :model="editingConfig" layout="vertical">
        <a-form-item label="名称" required>
          <a-input v-model:value="editingConfig.name" placeholder="如：K8s 生产集群" />
        </a-form-item>
        <a-row :gutter="16">
          <a-col :span="12">
          ...
          <a-col :span="12">
            <a-form-item v-if="editingConfig.type === 'k8s'" label="K8s 集群">
...
import { telegramBotApi, type TelegramBot } from '@/services/telegram'
...
const editingConfig = reactive<Partial<HealthCheckConfig>>({
  name: '', type: 'k8s', target_id: undefined, url: '', interval: 300, timeout: 10, retry_count: 3,
  enabled: true, alert_enabled: true, alert_platform: '', alert_bot_id: undefined
})

const currentAlertBots = computed(() =>
  editingConfig.alert_platform === 'telegram' ? telegramBots.value : [],
)
...
const typeLabels: Record<string, string> = { k8s: 'K8s', custom: '自定义' }
const typeColors: Record<string, string> = { k8s: 'purple', custom: 'blue' }
...
const fetchResources = async () => {
  try {
    const k8sRes = await k8sClusterApi.list()
...
const fetchBots = async () => {
  ...
  try {
    const r = await telegramBotApi.list(1, 500)
    if (r.code === 0 && r.data) telegramBots.value = r.data.list || []
...
const showConfigModal = (config?: HealthCheckConfig) => {
  if (config) {
    ...
    Object.assign(editingConfig, { name: '', type: 'k8s', target_id: undefined, url: '', interval: 300, timeout: 10, retry_count: 3, enabled: true, alert_enabled: true, alert_platform: '', alert_bot_id: undefined })
  }
