        ...
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="配置名称" required>
              <a-input v-model:value="form.name" placeholder="如：K8s Pod CrashLoopBackOff 告警" />
            </a-form-item>
          </a-col>
          <a-col :span="12">
          ...
          <a-space align="start" style="width: 100%">
            ...
            <div style="flex: 1">
              ...
              <a-input
                ...
                v-model:value="channel.receive_id"
                placeholder="Chat ID（与机器人 default_chat_id 一致时可留空）"
...
const typeLabels: Record<string, string> = { k8s_pod: 'K8s Pod', health_check: '健康检查', test: '测试告警' }
const typeColors: Record<string, string> = { k8s_pod: 'purple', health_check: 'blue', test: 'gray' }

const getTypeLabel = (type: string) => typeLabels[type] || type
const getTypeColor = (type: string) => typeColors[type] || 'default'

const getChannelLabel = (type: string) => ({ telegram: 'Telegram', email: '邮件' }[type] || type)
const getChannelColor = (type: string) => ({ telegram: 'geekblue', email: 'orange' }[type] || 'default')
...
const showModal = (record?: AlertConfig) => {
  if (record) {
    ...
    Object.assign(form, { name: '', type: 'k8s_pod', enabled: true, template_id: undefined, conditions: '', description: '' })
    resetConditionForm()
    channels.value = [{ type: 'telegram', url: '', receive_id: '' }]
  }
  modalVisible.value = true
}

const addChannel = () => {
  channels.value.push({ type: 'telegram', url: '', receive_id: '' })
}
