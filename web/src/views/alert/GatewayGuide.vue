<template>
  <div class="gateway-guide">
    <a-card :bordered="false" title="告警网关接入指南">
      ...
      <a-alert
        message="什么是告警网关？"
        description="告警网关是统一的告警接收入口。它负责接收来自 Prometheus、Grafana 或自定义脚本的告警请求，经过清洗、静默检查和路由匹配后，通过 Telegram 渠道发送通知。"
        type="info"
        show-icon
...
const sendTest = async () => {
  try {
    ...
    // 如果没有，这里可能需要调整 URL
    const res = await request.post<any, ApiResponse<GatewayTestResponse>>(`/gateway/event/${testSource.value}`, json)
