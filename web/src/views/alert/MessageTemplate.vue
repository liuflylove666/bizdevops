              ...
              <a-col :span="12">
                <a-form-item label="模板类型" required>
                  <a-select v-model:value="form.template_type">
                    ...
                    <a-select-option value="card">卡片消息 (JSON)</a-select-option>
...
const api = {
  list: (params: { page?: number; page_size?: number }) =>
    request.get<any, ApiResponse<TemplateListData>>('/notification/templates', { params }),
  create: (data: Partial<MessageTemplate>) =>
    request.post<any, ApiResponse<MessageTemplate>>('/notification/templates', data),
  update: (id: number, data: Partial<MessageTemplate>) =>
    request.put<any, ApiResponse<MessageTemplate>>(`/notification/templates/${id}`, data),
  delete: (id: number) =>
    request.delete<any, ApiResponse>(`/notification/templates/${id}`),
  preview: (data: { content?: string; data: Record<string, unknown> }) =>
    request.post<any, ApiResponse<TemplatePreviewData>>('/notification/templates/preview', data)
}
...
const typeLabels: Record<string, string> = { text: '文本', markdown: 'Markdown', card: '卡片', json: 'JSON' }
...
const preview = async () => {
  ...
  let dataObj = {}
  try {
    dataObj = JSON.parse(testDataJson.value) as Record<string, unknown>
  } catch (e) {
