...
const form = reactive<Partial<AlertEscalation>>({ name: '', level: 'error', delay_minutes: 30, platform: 'telegram', bot_id: undefined, enabled: true, description: '' })
...
const platformLabels: Record<string, string> = { telegram: 'Telegram' }
const platformColors: Record<string, string> = { telegram: 'geekblue' }
...
const currentBots = computed(() => (form.platform === 'telegram' ? telegramBots.value : []))
...
const showModal = (record?: AlertEscalation) => {
  ...
  else { editingId.value = undefined; Object.assign(form, { name: '', level: 'error', delay_minutes: 30, platform: 'telegram', bot_id: undefined, enabled: true, description: '' }) }
