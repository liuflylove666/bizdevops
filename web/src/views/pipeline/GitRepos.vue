<template>
  <div class="git-repos">
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space>
              ...
              <a-button type="link" size="small" @click="showBranches(record)">
                <BranchesOutlined /> 分支
              </a-button>
    ...
    <a-drawer v-model:open="branchDrawerVisible" :title="`${branchRepoName} 分支列表`" width="520">
      <a-spin :spinning="branchLoading">
        <a-empty v-if="!branchLoading && repoBranches.length === 0" description="暂无分支信息" />
        <a-list v-else :data-source="repoBranches" size="small">
          <template #renderItem="{ item }">
            <a-list-item>
              ...
              <template #actions>
                <span v-if="item.commit_sha" class="text-gray">{{ item.commit_sha.slice(0, 8) }}</span>
              </template>
            </a-list-item>
          </template>
        </a-list>
...
import { PlusOutlined, LinkOutlined, CopyOutlined, ApiOutlined, BranchesOutlined } from '@ant-design/icons-vue'
...
const testConnection = async (record: GitRepo) => {
  try {
    ...
    if (res?.data?.success) {
      message.success({ content: res?.data?.message || '连接成功', key: 'test' })
    } else {
...
const showBranches = async (record: GitRepo) => {
  branchDrawerVisible.value = true
  branchRepoName.value = record.name
  branchLoading.value = true
  repoBranches.value = []
  try {
    const res = await gitRepoApi.getBranches(record.id)
    repoBranches.value = res?.data || []
  } catch (error: any) {
    message.error(error?.message || '获取分支失败')
  } finally {
