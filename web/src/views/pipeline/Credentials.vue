      ...
      >
        ...
        <template v-if="form.type === 'token'">
          <a-form-item label="令牌">
            <a-input-password v-model:value="form.token" :placeholder="isEdit ? '留空则不修改' : '请输入访问令牌'" />
          </a-form-item>
        </template>
        ...
        <template v-if="form.type === 'ssh_key'">
          ...
          <a-form-item label="密钥密码" name="passphrase">
            <a-input-password v-model:value="form.passphrase" placeholder="密钥密码（如有）" />
          </a-form-item>
        </template>
        ...
        <template v-if="form.type === 'docker_config'">
          <a-form-item label="仓库地址">
            <a-input v-model:value="form.registry" placeholder="如: registry.example.com" />
          </a-form-item>
          <a-form-item label="用户名">
            <a-input v-model:value="form.username" placeholder="请输入用户名" />
          </a-form-item>
...
const buildCredentialJson = () => {
  switch (form.type) {
    ...
    case 'token':
      return JSON.stringify({ token: form.token })
    case 'ssh_key':
      return JSON.stringify({
...
const readFieldValueFromDom = (placeholder: string) => {
  const selector = `input[placeholder="${placeholder}"], textarea[placeholder="${placeholder}"]`
  const el = document.querySelector<HTMLInputElement | HTMLTextAreaElement>(selector)
  return el?.value || ''
}

const syncCredentialFieldsFromDom = () => {
  // Ant Design password inputs can be manipulated by browser automation before Vue receives an update.
  // Read the visible modal values as a last-mile fallback before validating and serializing.
  form.username ||= readFieldValueFromDom('请输入用户名')
  form.password ||= readFieldValueFromDom('请输入密码')
  form.password ||= readFieldValueFromDom('密码')
  form.token ||= readFieldValueFromDom('请输入访问令牌')
  form.ssh_key ||= readFieldValueFromDom('请粘贴 SSH 私钥内容')
  form.passphrase ||= readFieldValueFromDom('密钥密码（如有）')
  form.registry ||= readFieldValueFromDom('如: registry.example.com')
}
...
const validateCredentialData = () => {
  switch (form.type) {
    case 'username_password':
      ...
      if (!isEdit.value && !form.password) return '请输入密码'
      break
    case 'token':
      if (!isEdit.value && !form.token) return '请输入令牌'
      break
    case 'ssh_key':
      if (!isEdit.value && !form.ssh_key.trim()) return '请输入私钥'
      break
    case 'docker_config':
      if (!form.registry.trim()) return '请输入仓库地址'
      if (!form.username.trim()) return '请输入用户名'
      if (!isEdit.value && !form.password) return '请输入密码'
      break
  }
...
const handleSubmit = async () => {
  ...
  const validationMessage = validateCredentialData()
