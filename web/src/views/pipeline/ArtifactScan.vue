...
const isScanning = computed(() => {
  return (
    (scanResults.vulnerability.status as ScanStatus) === 'scanning' ||
    (scanResults.license.status as ScanStatus) === 'scanning' ||
    (scanResults.quality.status as ScanStatus) === 'scanning'
  )
})
  ...
  try {
    ...
    selectedScanTypes.value.forEach(type => {
      if (type === 'vulnerability') {
        scanResults.vulnerability.status = 'scanning' as ScanStatus
      } else if (type === 'license') {
        scanResults.license.status = 'scanning' as ScanStatus
      } else if (type === 'quality') {
        scanResults.quality.status = 'scanning' as ScanStatus
      }
    })
