<template>
  <div class="deploy-check">
    <a-page-header title="部署检查" sub-title="部署前置检查">
      <template #extra>
        <a-button type="primary" @click="$router.push('/argocd')">
        ...
        <a-result :status="checkResult.can_deploy ? 'success' : 'warning'" :title="checkResult.can_deploy ? '可以部署' : '存在问题'">
          <template #extra>
            <a-space v-if="checkResult.can_deploy">
              <a-button type="primary" @click="goToDeploy">查看交付</a-button>
              <a-button @click="goToHistory">交付记录</a-button>
            </a-space>
          </template>
        </a-result>
...
const goToHistory = () => {
  if (checkForm.application_id) {
    router.push(`/applications/${checkForm.application_id}#deploys`)
    return
  }
