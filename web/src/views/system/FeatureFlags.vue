...
const fetchStats = async () => {
  const res = await featureFlagApi.stats()
...
const saveFlag = async () => {
  ...
  try {
    ...
    if (editingFlag.value) {
      await featureFlagApi.update(form.name, payload)
    } else {
...
const toggleEnabled = async (record: FeatureFlag) => {
  try {
    await featureFlagApi.update(record.name, { is_enabled: record.is_enabled })
...
const deleteFlag = async (name: string) => {
  await featureFlagApi.delete(name)
...
const testFlag = async () => {
  ...
  testing.value = true
  try {
    const res = await featureFlagApi.check(testingFlag.value.name, {
      tenantId: testParams.tenant_id,
      userId: testParams.user_id
    })
    testResult.value = Boolean(res.data?.enabled)
  } finally {
    testing.value = false
  }
}
