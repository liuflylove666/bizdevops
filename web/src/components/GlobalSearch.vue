...
const getQuickNavItems = (): QuickNavItem[] => [
  { labelKey: 'search.quickPipeline', path: '/pipeline/list', icon: RocketOutlined, shortcut: 'G P' },
  { labelKey: 'search.quickK8s', path: '/k8s/clusters', icon: CloudOutlined, shortcut: 'G K' },
  { labelKey: 'search.quickApplications', path: '/applications', icon: AppstoreOutlined, shortcut: 'G A' },
  { labelKey: 'search.quickLogs', path: '/logs/center', icon: FileSearchOutlined, shortcut: 'G L' },
...
const groupedResults = computed(() => {
  ...
  const typeConfig: Record<string, { labelKey: string; icon: any }> = {
    ...
    release: { labelKey: 'search.release', icon: RocketOutlined },
    gitops_pr: { labelKey: 'search.gitopsPR', icon: AuditOutlined },
...
const handleSearch = () => {
  ...
  searchTimer = window.setTimeout(async () => {
    ...
    try {
      ...
      // 并行搜索多个资源（v2.0：增加 Release / GitOps PR 数据源）
      const [pipelinesRes, clustersRes, usersRes, releasesRes, gitopsPRsRes] = await Promise.allSettled([
        ...
        request.get('/releases', { params: { title: keyword.value, pageSize: 5 } }),
        request.get('/argocd/change-requests', { params: { keyword: keyword.value, pageSize: 5 } }),
      ])
      ...
      if (releasesRes.status === 'fulfilled') {
        const items = releasesRes.value?.data?.list || releasesRes.value?.data?.items || []
        items.forEach((item: any) => {
          searchResults.push({
            id: item.id,
            type: 'release',
            title: item.title,
            description: `${item.application_name || '-'} · ${item.env || '-'}`,
            status: item.status,
            path: `/releases/${item.id}`
          })
        })
      }

      // v2.0: 处理 GitOps PR 结果
      if (gitopsPRsRes.status === 'fulfilled') {
        const items = gitopsPRsRes.value?.data?.list || gitopsPRsRes.value?.data?.items || gitopsPRsRes.value?.data || []
        ;(Array.isArray(items) ? items : []).forEach((item: any) => {
          searchResults.push({
            id: item.id,
            type: 'gitops_pr',
            title: item.title,
            description: `${item.application_name || '-'} · ${item.env || '-'} · ${item.image_tag || ''}`,
            status: item.status,
            path: `/argocd?tab=changes&changeId=${item.id}`
          })
        })
      }
...
const handleKeydown = (e: KeyboardEvent) => {
  const maxIndex = keyword.value ? totalResults.value - 1 : quickNavItems.value.length - 1
  ...
  if (e.key === 'ArrowDown') {
    ...
    if (keyword.value) {
      ...
      const target = quickNavItems.value[selectedIndex.value]
      if (target) {
