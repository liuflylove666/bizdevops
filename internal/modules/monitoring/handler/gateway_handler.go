...
func NewGatewayHandler(db *gorm.DB) *GatewayHandler {
	h := &GatewayHandler{
		...
		incidentSvc:  incident.NewService(monRepo.NewIncidentRepository(db)),
	}
...
func (h *GatewayHandler) Ingest(c *gin.Context) {
	...
	for _, event := range events {
		// 2.0 Alert ↔ Incident 联动（v2.2）
		//    不论是否匹配路由/静默，都尝试 upsert incident，保证 DORA MTTR 数据完整
		h.syncIncident(c.Request.Context(), event)
...
// syncIncident 把 AlertEvent 同步到 incidents 表（v2.2）。
//
// firing  → 创建 / 升级 open incident
// resolved → 自动 Resolve 对应 incident（若存在）
// P3 的告警不建档，避免噪音污染 MTTR（可后续做成 flag 控制）
func (h *GatewayHandler) syncIncident(ctx context.Context, event monitoring.AlertEvent) {
	if h.incidentSvc == nil || event.Fingerprint == "" {
		return
	}
	// 过滤低级噪音：P3 （info 或无 severity）的 firing 不建 incident；resolved 仍要处理
	isResolved := strings.EqualFold(event.Status, "resolved") || strings.EqualFold(event.Status, "ok")
	if !isResolved {
		sev := mapIngestSeverity(event)
		if sev == monitoring.IncidentSeverityP3 {
			return
		}
	}
	inc, created, err := h.incidentSvc.UpsertFromAlert(ctx, &event)
	if err != nil {
		gatewayLog.Warn("UpsertFromAlert failed: %v", err)
		return
	}
	if created {
		gatewayLog.Info("incident created from alert fingerprint=%s id=%d", event.Fingerprint, inc.ID)
	}
}

// mapIngestSeverity 暴露给 gateway 自身的"是否 P3"判断（复用 service 层相同映射口径）。
func mapIngestSeverity(ev monitoring.AlertEvent) string {
	if ev.Labels != nil {
		if s := strings.ToUpper(strings.TrimSpace(ev.Labels["severity"])); s != "" {
