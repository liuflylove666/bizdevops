...
func (h *RBACHandler) InitDefaultPermissions() {
	...
	defaultRoles := []models.Role{
		...
		{Name: "operator", DisplayName: "运维人员", Description: "可以操作 K8s 等资源", IsSystem: true, Status: "active"},
	...
	if h.db.Where("name = ?", "developer").First(&devRole).Error == nil {
		var perms []models.Permission
		h.db.Where("action = ? OR (resource IN ? AND action = ?)", "read", []string{"app"}, "write").Find(&perms)
		var permIDs []uint
		for _, p := range perms {
			permIDs = append(permIDs, p.ID)
		}
