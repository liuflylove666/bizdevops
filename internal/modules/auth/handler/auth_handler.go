...
import (
	...
	authSvc "devops/internal/service/auth"
	"devops/pkg/dto"
	"devops/pkg/ioc"
...
func (h *AuthApiHandler) Init() error {
	...
	ldapSvc := authSvc.NewLDAPService(db, configRepo, userRepo, roleRepo, urRepo)

	h.handler = NewAuthHandler(db, cfg.JWTSecret, cfg.JWTExpiration, ldapSvc)
...
type AuthHandler struct {
	...
	jwtSecret     string
	jwtExpiration int
	ldapSvc       *authSvc.LDAPService
}

func NewAuthHandler(db *gorm.DB, jwtSecret string, jwtExpiration int, ldapSvc *authSvc.LDAPService) *AuthHandler {
	return &AuthHandler{db: db, jwtSecret: jwtSecret, jwtExpiration: jwtExpiration, ldapSvc: ldapSvc}
}
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	if localAuthOK {
		...
		ldapUser, err := h.ldapSvc.Authenticate(c.Request.Context(), req.Username, req.Password)
		if err != nil {
			if errors.Is(err, authSvc.ErrLDAPDisabled) {
				if localFound {
		...
		syncedUser, err := h.ldapSvc.SyncUserFromLDAP(c.Request.Context(), ldapUser)
		if err != nil {
			response.InternalError(c, "用户同步失败")
			return
		}
		user = *syncedUser
		if user.Status != "active" {
