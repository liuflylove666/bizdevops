...
func init() {
	ioc.Api.RegisterContainer("FeatureFlagHandler", &FeatureFlagApiHandler{})
}

// FeatureFlagApiHandler IoC 包装，负责在启动时挂载 /feature-flags 路由。
type FeatureFlagApiHandler struct {
	handler *FeatureFlagHandler
}

func (h *FeatureFlagApiHandler) Init() error {
	cfg, err := config.LoadConfig()
	if err != nil {
		return err
	}
	h.handler = NewFeatureFlagHandler(cfg.GetDB())

	root := cfg.Application.GinRootRouter()
	authed := root.Group("")
	authed.Use(middleware.AuthMiddleware())
	h.handler.RegisterRoutes(authed)
	return nil
}
