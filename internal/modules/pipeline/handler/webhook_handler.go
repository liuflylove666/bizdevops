...
func (h *WebhookHandler) HandleGitHub(c *gin.Context) {
	...
	if err != nil {
		...
		if saveErr := h.webhookService.SaveWebhookLog(c.Request.Context(), uint(repoID), wp, 0, err); saveErr != nil {
			log.WithField("error", saveErr).Error("保存 Webhook 日志失败")
		}
	...
	// 保存日志
	if saveErr := h.webhookService.SaveWebhookLog(c.Request.Context(), uint(repoID), wp, runID, err); saveErr != nil {
		log.WithField("error", saveErr).Error("保存 Webhook 日志失败")
	}
