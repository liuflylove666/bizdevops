...
func (h *GitHandler) GetBranches(c *gin.Context) {
	...
	if err != nil {
		response.FromError(c, apperrors.New(apperrors.ErrCodeBusiness, "获取分支列表失败："+err.Error()))
		return
	}
...
func (h *GitHandler) GetTags(c *gin.Context) {
	...
	if err != nil {
		response.FromError(c, apperrors.New(apperrors.ErrCodeBusiness, "获取 Tag 列表失败："+err.Error()))
		return
	}
