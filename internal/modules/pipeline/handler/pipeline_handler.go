...
func (h *PipelineApiHandler) Init() error {
	...
	h.handler = NewPipelineHandler(pipelineSvc, runSvc, credentialSvc, variableSvc)
...
func (h *PipelineHandler) CreatePipeline(c *gin.Context) {
	...
	if err := h.pipelineSvc.PrepareForCreate(c.Request.Context(), &req, role); err != nil {
...
func (h *PipelineHandler) UpdatePipeline(c *gin.Context) {
	...
	role := c.GetString("role")
	if err := h.pipelineSvc.PrepareForUpdate(c.Request.Context(), &req, role); err != nil {
		response.BadRequest(c, err.Error())
		return
	}
