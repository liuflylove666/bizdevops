...
func (s *DeployCheckService) PreCheck(ctx context.Context, req *dto.DeployPreCheckRequest) (*dto.DeployPreCheckResponse, error) {
	...
	envConfigured := false
	if err := s.db.WithContext(ctx).Where("app_id = ? AND env_name = ?", req.ApplicationID, req.EnvName).First(&appEnv).Error; err == nil {
		envConfigured = true
	}

	// 2. 检查 K8s 集群连接
	if envConfigured && appEnv.K8sClusterID != nil {
		resp.Checks = append(resp.Checks, s.checkK8sConnection(ctx, *appEnv.K8sClusterID))
	}
	...
	// 5. 检查当前 Pod 健康状态
	if envConfigured && appEnv.K8sClusterID != nil && appEnv.K8sNamespace != "" && appEnv.K8sDeployment != "" {
		check, warning := s.checkPodHealth(ctx, *appEnv.K8sClusterID, appEnv.K8sNamespace, appEnv.K8sDeployment)
	...
	// 6. 检查资源配额
	if envConfigured && appEnv.K8sClusterID != nil && appEnv.K8sNamespace != "" {
		check, warning := s.checkResourceQuota(ctx, *appEnv.K8sClusterID, appEnv.K8sNamespace)
		resp.Checks = append(resp.Checks, check)
		if warning != "" {
			resp.Warnings = append(resp.Warnings, warning)
		}
