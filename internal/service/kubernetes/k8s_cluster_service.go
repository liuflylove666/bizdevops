...
func BuildRestConfig(cluster *models.K8sCluster) (*rest.Config, error) {
	...
	applyAPIServerHostRewrite(config)
...
func (s *k8sClusterService) CreateK8sCluster(ctx context.Context, req *dto.CreateK8sClusterRequest) (*dto.K8sClusterResponse, error) {
	cluster := &models.K8sCluster{
		...
		Namespace:       firstNonEmpty(req.Namespace, "default"),
		...
		CheckTimeout:    defaultInt(req.CheckTimeout, 180),
	}
...
func (s *k8sClusterService) UpdateK8sCluster(ctx context.Context, id uint, req *dto.UpdateK8sClusterRequest) (*dto.K8sClusterResponse, error) {
	...
	cluster.Namespace = firstNonEmpty(req.Namespace, "default")
	cluster.Registry = req.Registry
	cluster.Repository = req.Repository
	cluster.Description = req.Description
	cluster.Status = req.Status
	cluster.IsDefault = req.IsDefault
	cluster.InsecureSkipTLS = req.InsecureSkipTLS
	cluster.CheckTimeout = defaultInt(req.CheckTimeout, 180)
...
func (s *k8sClusterService) buildResponse(cluster *models.K8sCluster) *dto.K8sClusterResponse {
	return &dto.K8sClusterResponse{
		...
		InsecureSkipTLS: cluster.InsecureSkipTLS,
		CheckTimeout:    cluster.CheckTimeout,
...
func (s *k8sClusterService) TestConnection(ctx context.Context, id uint) (*dto.ConnectionTestResult, error) {
	...
	if err != nil {
		...
		case strings.Contains(errStr, "connection refused"):
			result.Error = "连接被拒绝，请检查集群地址和端口是否正确。若 DevOps 部署在 Docker 中且 kubeconfig 的 server 为 127.0.0.1（如本机 kind），请在运行环境中设置 K8S_API_SERVER_HOST_REWRITE=host.docker.internal，并确保容器可解析该主机名（Linux 需在 compose 中为 devops 服务配置 extra_hosts: host.docker.internal:host-gateway）；或直接把 kubeconfig 中的 server 改为宿主机可达地址。"
