...
// sendNotification 通过默认 Telegram 机器人发送（模板为纯文本）
func (a *CostAlerter) sendNotification(ctx context.Context, templateName string, data map[string]interface{}) {
	bot, err := a.telegramBotRepo.GetDefault(ctx)
	if err != nil {
		a.log.WithField("error", err.Error()).Warn("加载默认 Telegram 机器人失败，跳过成本通知")
		return
	}
	if bot == nil {
		a.log.Warn("未配置默认 Telegram 机器人，跳过成本通知")
		return
	}
	token := strings.TrimSpace(bot.Token)
	chatID := strings.TrimSpace(bot.DefaultChatID)
	if token == "" || chatID == "" {
		a.log.Warn("Telegram 机器人 token 或 default_chat_id 为空，跳过成本通知")
		return
	}
	...
	if err := client.SendMessage(ctx, &telegram.SendMessageRequest{ChatID: chatID, Text: content}); err != nil {
		a.log.WithField("error", err.Error()).Warn("发送 Telegram 成本通知失败")
	}
}
