...
type NodeExecutor struct {
	...
	resultHandler    ApprovalResultHandler
}

// ApprovalResultHandler 审批结果处理器
type ApprovalResultHandler interface {
	OnApprovalApproved(ctx context.Context, instance *models.ApprovalInstance) error
	OnApprovalRejected(ctx context.Context, instance *models.ApprovalInstance) error
	OnApprovalCancelled(ctx context.Context, instance *models.ApprovalInstance, reason string) error
}
...
// SetResultHandler 设置审批结果处理器
func (e *NodeExecutor) SetResultHandler(handler ApprovalResultHandler) {
...
func (e *NodeExecutor) advanceInstance(ctx context.Context, instanceID uint) error {
	...
	if nextNodeInstance == nil {
		// 没有下一个节点，审批完成
		now := time.Now()
		if err := e.instanceRepo.UpdateStatus(ctx, instanceID, "approved", &now); err != nil {
			return err
		}
		return e.notifyApproved(ctx, instanceID)
	}
...
func (e *NodeExecutor) rejectInstance(ctx context.Context, instanceID uint) error {
	now := time.Now()
	if err := e.instanceRepo.UpdateStatus(ctx, instanceID, "rejected", &now); err != nil {
		return err
	}
	return e.notifyRejected(ctx, instanceID)
}

func (e *NodeExecutor) notifyApproved(ctx context.Context, instanceID uint) error {
	...
	return e.resultHandler.OnApprovalApproved(ctx, instance)
}

func (e *NodeExecutor) notifyRejected(ctx context.Context, instanceID uint) error {
	if e.resultHandler == nil {
		return nil
	}
	instance, err := e.instanceRepo.GetWithNodeInstances(ctx, instanceID)
	...
	return e.resultHandler.OnApprovalRejected(ctx, instance)
}

func (e *NodeExecutor) notifyCancelled(ctx context.Context, instanceID uint, reason string) error {
	if e.resultHandler == nil {
		return nil
	}
	instance, err := e.instanceRepo.GetWithNodeInstances(ctx, instanceID)
	...
	return e.resultHandler.OnApprovalCancelled(ctx, instance, reason)
}
