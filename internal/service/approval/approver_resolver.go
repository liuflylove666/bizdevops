...
func (r *ApproverResolver) ResolveApprovers(ctx context.Context, approverType, approvers string, appID uint) (string, error) {
	...
	case "user":
		// 兼容直接填写用户 ID 或用户名
		return r.resolveUsers(ctx, approvers)
...
func (r *ApproverResolver) resolveUsers(ctx context.Context, raw string) (string, error) {
	if raw == "" {
		return "", nil
	}
	...
	for _, part := range parts {
		...
		if err := r.db.WithContext(ctx).Where("username = ?", part).First(&user).Error; err != nil {
