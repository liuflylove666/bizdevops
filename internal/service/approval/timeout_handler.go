...
func (h *TimeoutHandler) handleTimeout(ctx context.Context, node *models.ApprovalNodeInstance) error {
	...
	case "auto_reject":
		...
		// 拒绝整个实例
		if err := h.instanceRepo.UpdateStatus(ctx, node.InstanceID, "rejected", &now); err != nil {
			return err
		}
		return h.nodeExecutor.notifyRejected(ctx, node.InstanceID)
	...
	case "auto_cancel":
		...
		if err := h.instanceRepo.UpdateStatus(ctx, node.InstanceID, "cancelled", &now); err != nil {
			return err
		}
		return h.nodeExecutor.notifyCancelled(ctx, node.InstanceID, "审批超时自动取消")
...
func (h *TimeoutHandler) advanceAfterTimeout(ctx context.Context, instanceID uint) error {
	...
	if nextNodeInstance == nil {
		// 没有下一个节点，审批完成
		now := time.Now()
		if err := h.instanceRepo.UpdateStatus(ctx, instanceID, "approved", &now); err != nil {
			return err
		}
		return h.nodeExecutor.notifyApproved(ctx, instanceID)
	}
