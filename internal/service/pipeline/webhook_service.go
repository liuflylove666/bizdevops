...
func (s *WebhookService) shouldTrigger(pipeline *models.Pipeline, wp *WebhookPayload) bool {
	triggerConfig, ok := parseTriggerConfig(pipeline)
	if !ok {
		return false
	}
...
func parseTriggerConfig(pipeline *models.Pipeline) (*TriggerConfig, bool) {
	if pipeline == nil {
		return nil, false
	}

	rawConfig := pipeline.TriggerConfigJSON
	if rawConfig == "" {
		rawConfig = pipeline.TriggerConfig
	}
	if rawConfig == "" {
		return nil, false
	}

	// 兼容当前前端/接口保存的 dto.TriggerConfig 结构。
	var modern dto.TriggerConfig
	if err := json.Unmarshal([]byte(rawConfig), &modern); err == nil {
		if modern.Webhook != nil {
			return &TriggerConfig{
				OnPush:   modern.Webhook.Enabled,
				OnPR:     modern.Webhook.Enabled,
				OnTag:    modern.Webhook.Enabled,
				Branches: modern.Webhook.BranchFilter,
				Tags:     modern.Webhook.BranchFilter,
			}, true
		}
	}

	// 回退兼容历史 on_push / branches 结构。
	var legacy TriggerConfig
	if err := json.Unmarshal([]byte(rawConfig), &legacy); err != nil {
		return nil, false
	}
	return &legacy, true
}
