...
func NewBuilderPodManager(db *gorm.DB) *BuilderPodManager {
	defaultConfig := loadDefaultBuilderPodConfigFromEnv()
	m := &BuilderPodManager{
		db:          db,
		clientMgr:   k8sservice.NewK8sClientManager(db),
		idleTimeout: time.Duration(defaultConfig.IdleTimeoutMinutes) * time.Minute,
...
func (m *BuilderPodManager) GetConfig() *BuilderPodConfig {
	...
	cfg := *sanitizeBuilderPodConfig(m.config)
...
func (m *BuilderPodManager) SetConfig(cfg *BuilderPodConfig) {
	...
	m.config = sanitizeBuilderPodConfig(cfg)
	// 同步空闲超时时间
	if m.config.IdleTimeoutMinutes > 0 {
		m.idleTimeout = time.Duration(m.config.IdleTimeoutMinutes) * time.Minute
	}
}
...
func loadDefaultBuilderPodConfigFromEnv() *BuilderPodConfig {
	return sanitizeBuilderPodConfig(&BuilderPodConfig{
		IdleTimeoutMinutes: getIntEnv("PIPELINE_BUILDER_IDLE_TIMEOUT_MINUTES", 30),
		StorageType:        getStringEnv("PIPELINE_BUILDER_STORAGE_TYPE", "hostpath"),
		PVCName:            getStringEnv("PIPELINE_BUILDER_PVC_NAME", "devops-workspace-shared"),
		PVCSizeGi:          getIntEnv("PIPELINE_BUILDER_PVC_SIZE_GI", 10),
		StorageClass:       strings.TrimSpace(os.Getenv("PIPELINE_BUILDER_STORAGE_CLASS")),
		AccessMode:         getStringEnv("PIPELINE_BUILDER_ACCESS_MODE", "ReadWriteOnce"),
		HostPath:           getStringEnv("PIPELINE_BUILDER_HOST_PATH", "/tmp/devops-workspace"),
		CPURequest:         getStringEnv("PIPELINE_BUILDER_CPU_REQUEST", "100m"),
		CPULimit:           getStringEnv("PIPELINE_BUILDER_CPU_LIMIT", "2"),
		MemoryRequest:      getStringEnv("PIPELINE_BUILDER_MEMORY_REQUEST", "128Mi"),
		MemoryLimit:        getStringEnv("PIPELINE_BUILDER_MEMORY_LIMIT", "1Gi"),
	})
}

func sanitizeBuilderPodConfig(cfg *BuilderPodConfig) *BuilderPodConfig {
	if cfg == nil {
		cfg = &BuilderPodConfig{}
	}

	sanitized := *cfg
	if sanitized.IdleTimeoutMinutes <= 0 {
		sanitized.IdleTimeoutMinutes = 30
	}

	switch strings.ToLower(strings.TrimSpace(sanitized.StorageType)) {
	case "hostpath":
		sanitized.StorageType = "hostpath"
	case "emptydir":
		sanitized.StorageType = "emptydir"
	default:
		sanitized.StorageType = "pvc"
	}

	if strings.TrimSpace(sanitized.PVCName) == "" {
		sanitized.PVCName = "devops-workspace-shared"
	}
	if sanitized.PVCSizeGi <= 0 {
		sanitized.PVCSizeGi = 10
	}
	if strings.TrimSpace(sanitized.AccessMode) == "" {
		sanitized.AccessMode = "ReadWriteOnce"
	}
	if sanitized.AccessMode != "ReadWriteMany" && sanitized.AccessMode != "ReadWriteOnce" {
		sanitized.AccessMode = "ReadWriteOnce"
	}
	if strings.TrimSpace(sanitized.HostPath) == "" {
		sanitized.HostPath = "/tmp/devops-workspace"
	}
	if strings.TrimSpace(sanitized.CPURequest) == "" {
		sanitized.CPURequest = "100m"
	}
	if strings.TrimSpace(sanitized.CPULimit) == "" {
		sanitized.CPULimit = "2"
	}
	if strings.TrimSpace(sanitized.MemoryRequest) == "" {
		sanitized.MemoryRequest = "128Mi"
	}
	if strings.TrimSpace(sanitized.MemoryLimit) == "" {
		sanitized.MemoryLimit = "1Gi"
	}
