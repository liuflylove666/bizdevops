...
import (
	...
	securitysvc "devops/internal/service/security"
	"devops/pkg/dto"
	"devops/pkg/logger"
)
...
func (e *ExecutorEngine) Execute(ctx context.Context, runID uint) error {
	...
	if pipeline.GitRepoID != nil && *pipeline.GitRepoID > 0 {
		...
		if err := e.db.First(&gitRepo, *pipeline.GitRepoID).Error; err == nil {
			env["__GIT_REPO_URL__"] = e.buildRuntimeGitRepoURL(ctx, &gitRepo)
...
stageLoop:
	...
	if finalStatus == "success" {
		...
		// GitOps 自动交接会直接更新 pipeline_runs 摘要字段，刷新后再保存最终状态，避免被旧内存对象覆盖。
		if reloadErr := e.db.First(&run, run.ID).Error; reloadErr != nil {
			log.WithError(reloadErr).Warn("刷新流水线运行记录失败，GitOps 摘要可能无法及时回显")
		}
	}
	run.Status = finalStatus
	e.db.Save(&pipeline)
...
func (e *ExecutorEngine) buildRuntimeGitRepoURL(ctx context.Context, repo *models.GitRepository) string {
	...
	// 流水线在容器/Builder Pod 中运行时，localhost 应改写到宿主机。
	...
	cred, err := NewCredentialService(e.db).GetDecryptedData(ctx, *repo.CredentialID)
	if err != nil {
		return parsed.String()
	}
	...
	if password == "" {
		password = strings.TrimSpace(cred.Token)
	}
	if password == "" {
		return parsed.String()
	}
	if username == "" && repo.Provider == "gitlab" {
		username = "oauth2"
	}
	if username != "" {
...
func (e *ExecutorEngine) executeStage(ctx context.Context, runID uint, stage dto.Stage, env map[string]string) string {
	...
	if stage.Parallel {
		...
		for _, step := range stage.Steps {
			...
			go func(s dto.Step) {
				defer wg.Done()
				status := e.executeStep(ctx, runID, stageRun.ID, e.resolveStepConfig(s, env), env)
				mu.Lock()
	...
	stepLoop:
		for _, step := range stage.Steps {
			...
			status := e.executeStep(ctx, runID, stageRun.ID, e.resolveStepConfig(step, env), env)
			if status == "failed" {
				finalStatus = "failed"
				break stepLoop
			}
...
// executeStep 执行步骤
func (e *ExecutorEngine) executeStep(ctx context.Context, runID uint, stageRunID uint, step dto.Step, env map[string]string) string {
	...
	if step.Type == "k8s_deploy" {
		msg := "K8s 直发步骤已下线，请改用 Argo CD / GitOps 交接流水线"
	...
	if err != nil {
		...
		if gateErr := e.enforceImageScanGate(stepCtx, runID, step, env, result); gateErr != nil {
			stepRun.Status = "failed"
			if result != nil && strings.TrimSpace(result.Logs) != "" {
				stepRun.Logs = result.Logs + "\n" + gateErr.Error()
			} else {
				stepRun.Logs = gateErr.Error()
			}
		}
	}
...
func (e *ExecutorEngine) enforceImageScanGate(ctx context.Context, runID uint, step dto.Step, env map[string]string, result *executor.StepResult) error {
	if step.Type != "docker_push" || e.db == nil {
		return nil
	}

	image := strings.TrimSpace(readStepConfigString(step.Config, "image"))
	if image == "" {
		return fmt.Errorf("镜像扫描门禁失败：未找到镜像地址，无法执行安全扫描")
	}

	registryID, err := readStepConfigUint(step.Config, "registry_id")
	if err != nil {
		return fmt.Errorf("镜像扫描门禁失败：registry_id 配置无效: %w", err)
	}

	scanSvc := securitysvc.NewImageScannerService(e.db)
	scanResult, err := scanSvc.ScanImage(ctx, &dto.ScanImageRequest{
		Image:           image,
		RegistryID:      registryID,
		ApplicationName: firstNonEmptyString(strings.TrimSpace(env["APP_NAME"]), strings.TrimSpace(env["APPLICATION_NAME"])),
		PipelineRunID:   runID,
	})
	if err != nil {
		e.updateRunImageScanSummary(runID, image, "failed", "", 0, 0, 0, 0)
		return fmt.Errorf("镜像扫描门禁失败：无法完成镜像 `%s` 的漏洞扫描: %w", image, err)
	}
	e.updateRunImageScanSummary(
		runID,
		image,
		scanResult.Status,
		scanResult.RiskLevel,
		scanResult.VulnSummary.Critical,
		scanResult.VulnSummary.High,
		scanResult.VulnSummary.Medium,
		scanResult.VulnSummary.Low,
	)

	if result != nil {
		var builder strings.Builder
		if strings.TrimSpace(result.Logs) != "" {
			builder.WriteString(result.Logs)
			builder.WriteString("\n")
		}
		builder.WriteString(formatImageScanSummary(scanResult))
		result.Logs = builder.String()
	}

	if scanResult.Status != "completed" {
		return fmt.Errorf("镜像扫描门禁失败：镜像 `%s` 扫描未完成，状态=%s，错误=%s", image, scanResult.Status, scanResult.ErrorMessage)
	}
	if scanResult.VulnSummary.Critical > 0 || scanResult.VulnSummary.High > 0 {
		return fmt.Errorf(
			"镜像扫描门禁未通过：镜像 `%s` 检测到严重 %d 个、高危 %d 个漏洞，已阻断后续交付",
			image,
			scanResult.VulnSummary.Critical,
			scanResult.VulnSummary.High,
		)
...
func (e *ExecutorEngine) updateRunImageScanSummary(runID uint, image, status, riskLevel string, critical, high, medium, low int) {
	if e.db == nil || runID == 0 {
		return
	}
	_ = e.db.Model(&models.PipelineRun{}).
		Where("id = ?", runID).
		Updates(map[string]interface{}{
			"scanned_image":         strings.TrimSpace(image),
			"image_scan_status":     strings.TrimSpace(status),
			"image_scan_risk_level": strings.TrimSpace(riskLevel),
			"image_scan_critical":   critical,
			"image_scan_high":       high,
			"image_scan_medium":     medium,
			"image_scan_low":        low,
		}).Error
}

func formatImageScanSummary(result *dto.ScanResultResponse) string {
	if result == nil {
		return "镜像扫描已跳过：无扫描结果"
	}
	return fmt.Sprintf(
		"=== 镜像安全扫描 ===\n镜像: %s\n风险等级: %s\n严重: %d, 高危: %d, 中危: %d, 低危: %d\n扫描状态: %s\n",
		result.Image,
...
func readStepConfigString(config map[string]interface{}, key string) string {
	if config == nil {
		return ""
	}
...
func readStepConfigUint(config map[string]interface{}, key string) (uint, error) {
	if config == nil {
		return 0, nil
	}
	raw, ok := config[key]
...
func (e *ExecutorEngine) resolveStepConfig(step dto.Step, env map[string]string) dto.Step {
	if step.Config == nil {
		step.Config = map[string]interface{}{}
		return step
	}
	if expanded, ok := expandPipelineValue(step.Config, env).(map[string]interface{}); ok {
		step.Config = expanded
	}
	return step
}

func expandPipelineValue(value interface{}, env map[string]string) interface{} {
	switch v := value.(type) {
	case string:
	...
	case []interface{}:
		...
		for i, item := range v {
			items[i] = expandPipelineValue(item, env)
		}
		return items
	case map[string]interface{}:
		out := make(map[string]interface{}, len(v))
		for k, item := range v {
			out[k] = expandPipelineValue(item, env)
		}
		return out
	default:
		return value
