...
func NewTrivyScanner() *TrivyScanner {
	...
	if err := exec.Command("docker", "version").Run(); err == nil {
		return &TrivyScanner{trivyPath: "aquasec/trivy:latest", useDocker: true}
	}
...
func (s *TrivyScanner) Scan(ctx context.Context, image string, registry *models.ImageRegistry) (*ScanResult, error) {
	...
	cmd := s.buildScanCommand(ctx, args, registry)
	...
	if err != nil {
		// 检查是否是Trivy未安装
		if strings.Contains(err.Error(), "executable file not found") && !s.useDocker {
			log.Error("Trivy未安装")
...
func (s *TrivyScanner) buildScanCommand(ctx context.Context, args []string, registry *models.ImageRegistry) *exec.Cmd {
	if s.useDocker {
		dockerArgs := []string{
			"run", "--rm",
		...
		if registry != nil && registry.Username != "" {
			dockerArgs = append(dockerArgs,
				"-e", fmt.Sprintf("TRIVY_USERNAME=%s", registry.Username),
				"-e", fmt.Sprintf("TRIVY_PASSWORD=%s", registry.Password),
			)
		}
		dockerArgs = append(dockerArgs, s.trivyPath)
	...
	if registry != nil && registry.Username != "" {
		cmd.Env = append(cmd.Environ(),
			fmt.Sprintf("TRIVY_USERNAME=%s", registry.Username),
			fmt.Sprintf("TRIVY_PASSWORD=%s", registry.Password),
		)
	}
	return cmd
}
