...
func (s *RegistryService) List(ctx context.Context) ([]dto.ImageRegistryItem, error) {
	...
	for _, r := range registries {
		...
		if url == "" {
			url = strings.TrimSpace(r.Address)
		}
...
func (s *RegistryService) Create(ctx context.Context, req *dto.ImageRegistryRequest) error {
	...
	encryptedPassword, err := encryptRegistryPassword(req.Password)
	if err != nil {
		return fmt.Errorf("加密仓库密码失败: %w", err)
	}
...
func (s *RegistryService) Get(ctx context.Context, id uint) (*models.ImageRegistry, error) {
	...
	if strings.TrimSpace(registry.URL) == "" {
		registry.URL = strings.TrimSpace(registry.Address)
	}
	return &registry, nil
}

// TestConnection 测试连接
func (s *RegistryService) TestConnection(ctx context.Context, req *dto.ImageRegistryTestRequest) error {
	log := logger.L().WithField("url", req.URL)

	client := &http.Client{Timeout: 10 * time.Second}

	password, err := s.resolveRegistryPassword(ctx, req)
	if err != nil {
		return err
	}
...
func (s *RegistryService) ListImages(ctx context.Context, registryID uint) ([]dto.RegistryImageItem, error) {
	...
	password, err := decryptRegistryPassword(registry.Password)
	if err != nil {
		// 兼容历史明文数据
	...
	if registry.Username != "" && password != "" {
		auth := base64.StdEncoding.EncodeToString([]byte(registry.Username + ":" + password))
...
func (s *RegistryService) resolveRegistryPassword(ctx context.Context, req *dto.ImageRegistryTestRequest) (string, error) {
	...
	registry, err := s.Get(ctx, req.RegistryID)
	if err != nil {
		return "", err
	}
