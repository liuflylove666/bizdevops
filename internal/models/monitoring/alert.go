...
type AlertConfig struct {
	...
	Type        string    `gorm:"size:50;not null;index" json:"type"` // 类型: k8s_pod/health_check
	Enabled     bool      `gorm:"default:true" json:"enabled"`        // 是否启用
	Platform    string    `gorm:"size:50;not null" json:"platform"`   // 通知平台（升级规则等）：telegram
...
type AlertSilence struct {
	...
	Type      string    `gorm:"size:50;not null;index" json:"type"`              // 类型: k8s_pod/health_check/all
