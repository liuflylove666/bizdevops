...
// PipelineTemplateFavorite 模板收藏
type PipelineTemplateFavorite struct {
	ID         uint64    `gorm:"primaryKey;autoIncrement" json:"id"`
	TemplateID uint64    `gorm:"index:idx_template_user,unique;not null;comment:模板ID" json:"template_id"`
	UserID     uint      `gorm:"index:idx_template_user,unique;not null;comment:用户ID" json:"user_id"`
	CreatedAt  time.Time `json:"created_at"`
}

func (PipelineTemplateFavorite) TableName() string { return "pipeline_template_favorites" }
...
type PipelineStepTemplate struct {
	...
	StepType     string    `gorm:"size:50;not null;index;comment:步骤类型" json:"step_type"` // git, shell, container, docker_build, etc.
...
var BuiltinPipelineTemplates = []PipelineTemplate{
	{
		Name:        "git-build-image-deploy",
		Description: "通用 Git 项目标准流水线，包含拉取代码、编译/测试、生成镜像、推送镜像、GitOps 交接五个阶段",
		Category:    "build",
		Language:    "universal",
		Framework:   "docker-gitops",
		ConfigJSON: `{
			"stages": [
				{
					"id": "checkout",
					...
					"steps": [
						{
							...
							"config": {
								...
								"work_dir": ".",
								"commands": [
									"if [ -n \"$BUILD_PREPARE_COMMAND\" ]; then sh -c \"$BUILD_PREPARE_COMMAND\"; fi",
									"if [ -n \"$TEST_COMMAND\" ]; then sh -c \"$TEST_COMMAND\"; fi",
									"if [ -n \"$BUILD_COMMAND\" ]; then sh -c \"$BUILD_COMMAND\"; else echo '未配置 BUILD_COMMAND，跳过编译'; fi"
								]
							}
						}
					]
					...
					"steps": [
						{
							...
							"type": "shell",
							"config": {
								"script": "echo 'GitOps handoff is handled automatically after successful run when AUTO_GITOPS_HANDOFF=true'",
								"commands": [
									"echo 'GitOps handoff is handled automatically after successful run when AUTO_GITOPS_HANDOFF=true'"
								]
							}
						}
					]
			...
			"variables": [
				{"name": "BUILD_PREPARE_COMMAND", "value": ""},
				{"name": "TEST_COMMAND", "value": ""},
				{"name": "BUILD_COMMAND", "value": ""},
				{"name": "DOCKER_REGISTRY", "value": ""},
				{"name": "IMAGE_NAME", "value": ""},
				{"name": "IMAGE_TAG", "value": "latest"},
				{"name": "REGISTRY_ID", "value": ""},
				{"name": "AUTO_GITOPS_HANDOFF", "value": "false"},
				{"name": "APP_NAME", "value": ""},
				{"name": "DEPLOY_ENV", "value": ""},
				{"name": "GITOPS_REPO_ID", "value": ""},
				{"name": "ARGOCD_APPLICATION_ID", "value": ""},
				{"name": "GITOPS_FILE_PATH", "value": ""},
				{"name": "GITOPS_TARGET_BRANCH", "value": ""}
			]
		}`,
		IsBuiltin: true,
		IsPublic:  true,
	...
	{
		Name:        "java-maven-k8s",
		Description: "Java Maven 项目构建镜像并交接到 GitOps",
		Category:    "build",
		Language:    "java",
		Framework:   "spring",
		ConfigJSON: `{
			"stages": [
				...
				},
				{
					...
					"name": "GitOps 交接",
					"steps": [
						{"id": "gitops_handoff", "name": "GitOps Handoff", "type": "shell", "config": {"script": "echo 'GitOps handoff is handled automatically after successful run when AUTO_GITOPS_HANDOFF=true'", "commands": ["echo 'GitOps handoff is handled automatically after successful run when AUTO_GITOPS_HANDOFF=true'"]}}
					]
				}
			],
			"variables": [
	...
	{
		Name:        "go-k8s",
		Description: "Go 项目测试、构建镜像并交接到 GitOps",
		Category:    "build",
		Language:    "go",
		Framework:   "gin",
		ConfigJSON: `{
			"stages": [
				{
					...
					"name": "单元测试",
					"steps": [
						{"id": "go_test", "name": "Go Test", "type": "container", "config": {"image": "golang:1.25-alpine", "commands": ["go test ./..."]}}
					]
				},
				{
					"id": "docker",
					"name": "镜像构建与推送",
					"steps": [
						{"id": "build_and_push_image", "name": "Build And Push Image", "type": "docker_build", "config": {}}
					]
				},
				{
					"id": "deploy",
	...
	{
		Name:        "nodejs-k8s",
		Description: "Node.js 项目构建镜像并交接到 GitOps",
		Category:    "build",
		Language:    "nodejs",
		Framework:   "express",
		ConfigJSON: `{
	...
	{
		Name:        "python-k8s",
		Description: "Python 项目构建镜像并交接到 GitOps",
		Category:    "build",
		Language:    "python",
		Framework:   "django",
		ConfigJSON: `{
	...
	{
		Name:        "rust-k8s",
		Description: "Rust 项目测试、构建镜像并交接到 GitOps",
		Category:    "build",
		Language:    "rust",
		Framework:   "axum",
		ConfigJSON: `{
			"stages": [
				{
					...
					"name": "单元测试",
					"steps": [
						{"id": "cargo_test", "name": "Cargo Test", "type": "container", "config": {"image": "rust:1.87-alpine", "commands": ["apk add --no-cache musl-dev pkgconfig openssl-dev build-base", "cargo test"]}}
					]
				},
				{
					"id": "lint",
					"name": "代码检查",
					"steps": [
						{"id": "cargo_clippy", "name": "Cargo Clippy", "type": "container", "config": {"image": "rust:1.87-alpine", "commands": ["apk add --no-cache musl-dev pkgconfig openssl-dev build-base", "cargo clippy --all-targets --all-features -- -D warnings"]}}
					]
				},
				{
					"id": "docker",
				...
				"variables": [
					{"name": "BUILD_COMMAND", "value": "cargo build --release"},
					{"name": "RUSTFLAGS", "value": ""},
...
var BuiltinStepTemplates = []PipelineStepTemplate{
	...
	{
		Name:        "GitOps 交接",
		Description: "流水线成功后由 GitOps 自动创建变更请求",
		StepType:    "shell",
		Category:    "deploy",
		IconName:    "gitops",
		ConfigSchema: `{
			"type": "object",
			"properties": {
				"script": {"type": "string", "title": "说明脚本", "default": "echo 'GitOps handoff is handled automatically after successful run when AUTO_GITOPS_HANDOFF=true'"},
				"commands": {"type": "array", "items": {"type": "string"}, "title": "命令列表"}
			}
		}`,
		DefaultJSON: `{
			"script": "echo 'GitOps handoff is handled automatically after successful run when AUTO_GITOPS_HANDOFF=true'",
			"commands": ["echo 'GitOps handoff is handled automatically after successful run when AUTO_GITOPS_HANDOFF=true'"]
		}`,
		IsBuiltin: true,
		SortOrder: 6,
	},
	{
		...
		ConfigSchema: `{
			"type": "object",
			"properties": {
				"type": {"type": "string", "title": "通知类型", "enum": ["telegram", "webhook", "email"]},
				"webhook_url": {"type": "string", "title": "Telegram Bot Token 或 Webhook URL"},
