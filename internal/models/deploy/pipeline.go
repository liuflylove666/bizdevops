...
type Pipeline struct {
	...
	ApplicationID     *uint      `gorm:"index:idx_application" json:"application_id"`           // 关联应用ID
	ApplicationName   string     `gorm:"size:100;index:idx_application_name" json:"application_name"`
	Env               string     `gorm:"size:50;index:idx_env" json:"env"` // 交付环境
	SourceTemplateID  *uint      `gorm:"index:idx_source_template" json:"source_template_id"`   // 来源模板ID
...
type PipelineRun struct {
	...
	ImageScanStatus       string     `gorm:"size:30" json:"image_scan_status"`
	ScannedImage          string     `gorm:"size:500" json:"scanned_image"`
	ImageScanRiskLevel    string     `gorm:"size:30" json:"image_scan_risk_level"`
	ImageScanCritical     int        `gorm:"default:0" json:"image_scan_critical"`
	ImageScanHigh         int        `gorm:"default:0" json:"image_scan_high"`
	ImageScanMedium       int        `gorm:"default:0" json:"image_scan_medium"`
	ImageScanLow          int        `gorm:"default:0" json:"image_scan_low"`
	GitOpsChangeRequestID *uint      `gorm:"index:idx_gitops_change_request" json:"gitops_change_request_id"`
	GitOpsHandoffStatus   string     `gorm:"size:30" json:"gitops_handoff_status"`
	GitOpsHandoffMessage  string     `gorm:"type:text" json:"gitops_handoff_message"`
...
type WebhookLog struct {
	...
	PipelineRun uint      `gorm:"column:pipeline_run_id;index:idx_pipeline_run" json:"pipeline_run"` // 触发的流水线运行ID
